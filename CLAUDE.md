...
- **Phase 9 (firm layer foundation) — IN FLIGHT (foundation landed 2026-06-15).** Branch `claude/phase9-firm-foundation`, PR #49 (draft, CI green). Shipped on the branch: the idempotent migration `20260615172439_phase9_firm_layer_foundation.sql` (firms/firm_members/firm_activity_log/deleted_firms; workspace `firm_id`/`firm_child_label`/`restrict_firm_access`/`firm_joined_at`; firm-aware `is_workspace_member`; `is_firm_member`/`is_firm_admin`; counter/plan-lock + entitlement/binding guard triggers; owner-only-mints-admins RLS; `v_firm_child_usage`/`v_firm_billing_period_summary` security_invoker views) **applied to staging + isolation-matrix-verified**; 4 service-role edge functions (`create-firm`/`add-firm-member`/`bind-workspace-to-firm`/`release-workspace-from-firm`) deployed; Node⇄Deno firm-access helpers (`src/lib/firmAccess.ts` + `_shared/firm_access.ts`); stripe-webhook firm branch (`applyFirmSubscription`, routed on `metadata.firm_id` — **coded, not yet deployed**, inert until firm subs exist); minimal frontend (firm-bound Billing banner + firm-grouped workspace selector). **Deferred follow-ons:** deploy the webhook firm branch from disk; `set-firm-access` (audited `restrict_firm_access` toggle) + `delete-firm` (forensic) edge fns; firm edge functions leak raw DB constraint names (KNOWN_ISSUES #102, LOW); **firm-bound billing lockdown — HIGH items RESOLVED (KNOWN_ISSUES #103 items 1–3): server guards landed in Phase 10 (create-checkout/customer-portal/manage-document-pack reject firm-bound with `firm_managed`); client affordances closed 2026-06-21 by audit D1 (AccountSettings payment button + all capacity-pack doors firm-gated). #103 items 4–8 (MED/LOW firm-billing UX polish) remain open.** **Not yet merged to main.**
