...
- **Tier 3 Contextual Intelligence (Sonnet):** Event-driven. Fires on state changes (new lease confirmed, amendment confirmed, renewal window opens). Compares against portfolio context; generates insights only when meaningful. v1 is manual-trigger (`generate-lease-insights`, Business-gated); auto-trigger is v2.
