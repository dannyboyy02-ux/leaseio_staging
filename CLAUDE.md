...
**Phase-7 chain-column remediation — RESOLVED in code 2026-06-19 (#111, merged to `main`).** The repo's `resolve-approval-chain` already sets the Phase-7 columns (`effective_assignee_user_id`/`assignee_resolution_source`/`pending_since`) at chain creation; #111 added a frontier-aware backfill for existing chains (migration `20260618150000`) + an atomic reroute-reconcile RPC (C5). **The #84/#111 function redeploy is DONE** — verified 2026-07-12: the *deployed* `resolve-approval-chain` (edge-function **v37**) already writes all three Phase-7 columns and calls `reroute_reconcile_chain_steps` + `forceConceptReactivation`; the backfill (`20260619131515`) + reconcile RPC (`20260619131540`) are applied to staging. The old "deployed copy is pre-#84 / writes NULL Phase-7 columns" note (and the `#59` DEPLOY_RUNBOOK step) is **stale — disregard it.** The ONE remaining undeployed diff is the **2026-07-12 canonical asset-type matcher** (`canonicalAssetType`, KNOWN_ISSUES walkthrough item): the repo `resolve-approval-chain` now folds `property`≡`real_estate`≡`"Real Estate"` in `matchPolicy()`, but the deployed v37 still does exact-match. The SQL/tester side (`preview_policy_resolution` via migration `…140000`) IS live, so the tester is canonical while the live matcher lags **until `resolve-approval-chain` is redeployed** — a verified-SAFE single-change redeploy (only ~20 lines differ, no new DB dependency, shared modules byte-identical). Deploy it via the owner's Supabase CLI (`supabase functions deploy resolve-approval-chain`); no CI auto-deploy exists for edge functions.
