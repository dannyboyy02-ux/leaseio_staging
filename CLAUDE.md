...
**Phase-7 chain-column remediation — RESOLVED in code 2026-06-19 (#111, merged to `main`).** The repo's `resolve-approval-chain` already sets the Phase-7 columns (`effective_assignee_user_id`/`assignee_resolution_source`/`pending_since`) at chain creation; #111 added a frontier-aware backfill for existing chains (migration `20260618150000`) + an atomic reroute-reconcile RPC (C5). The once-"permanently deferred" `resolve-approval-chain` redeploy is no longer a standing deferral — it's the final **operator** deploy step (apply the backfill + redeploy the function, which is also the long-deferred #84 redeploy), tracked in the `#59` step of `docs/DEPLOY_RUNBOOK_2026-06-18.md`. Until that deploy runs, the *deployed* (pre-#84) copy keeps writing NULL Phase-7 columns on new chains; after it, new + existing chains are both correct.
