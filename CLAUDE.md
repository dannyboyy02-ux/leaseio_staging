...
- **Stripe payment-method handling — two distinct rules by surface** (`get-billing-summary`, `create-workspace`, any new billing surface): Stripe Checkout defaults to **Link**, and customers also use Apple/Google Pay and ACH — all non-`card` PaymentMethod types. (1) **DISPLAY / read surfaces** (Billing tab, summaries): NEVER filter `paymentMethods.list({ type: "card" })` or read only `pm.card` — that silently drops non-card methods and showed "no payment method" for a paying customer (incident 2026-07-11). Resolve with NO type filter and map every method through `describePaymentMethod()` (`src/lib/paymentMethodDisplay.ts` + Deno mirror `supabase/functions/_shared/payment_method.ts`), which returns a labeled descriptor for any type and never null for a present method. (2) **CARD-SPECIFIC CHARGE flows** (e.g. `create-workspace`'s $499 add-workspace, confirmed client-side with Stripe.js `confirmCardPayment`): these legitimately REQUIRE a card and must **filter `type:"card"` and reject non-card EARLY** (`no_card_on_file`) — never accept a method the confirmation path can't complete, or the caller passes eligibility then fails after a pending workspace/PaymentIntent is created and rolled back (PR #81). Supporting non-card methods there needs a type-branched confirmation flow (tracked enhancement). Also: `invoice_settings.default_payment_method` is empty after a Checkout subscription (Checkout sets the method on the *subscription*), so don't rely on it alone. **Any billing/payment surface must be exercised with a real Stripe test transaction — including a Link/wallet method — before it's called done** (Definition-of-Done gate; the audit missed this by reading the code without driving a payment).
