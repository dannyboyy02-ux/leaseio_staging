...
- **Stripe payment-method display/resolution** (`get-billing-summary`, `create-workspace`, any new billing surface): NEVER filter `paymentMethods.list({ type: "card" })` or read only `pm.card`. Stripe Checkout defaults to **Link**, and customers also use Apple/Google Pay and ACH — all non-`card` PaymentMethod types. Filtering `type:"card"` silently drops them: the Billing tab showed "no payment method" for a paying customer and `create-workspace` rejected them with `no_card_on_file` (incident 2026-07-11, KNOWN_ISSUES). Resolve with NO type filter and map every method through `describePaymentMethod()` (`src/lib/paymentMethodDisplay.ts` + Deno mirror `supabase/functions/_shared/payment_method.ts`), which returns a labeled descriptor for any type and never null for a present method. Also: `invoice_settings.default_payment_method` is empty after a Checkout subscription (Checkout sets the method on the *subscription*), so don't rely on it alone. **Any billing/payment surface must be exercised with a real Stripe test transaction — including a Link/wallet method — before it's called done** (Definition-of-Done gate; the audit missed this by reading the code without driving a payment).
