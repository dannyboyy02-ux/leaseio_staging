...
**Locales (update together — ENFORCED):** `src/locales/{en,es}/common.json`. Any new or changed user-facing string ships through i18n with keys in BOTH files in the same commit (formal-usted Spanish, matching the surrounding tone) — never a hardcoded English literal in JSX/toasts. `src/lib/__tests__/localeParity.test.ts` fails CI on key drift, empty values, or `{{placeholder}}` mismatches between the two files.
