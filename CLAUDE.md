...
- **Phase 10 (firm UX) — IN FLIGHT (CP1 landed 2026-06-15).** Branch `claude/phase10-firm-ux`. The 5 kickoff decisions were taken as recommended. **CP1 applied to staging:** migration `20260616120000_phase10_firm_ux.sql` (profiles.current_firm_id, firm_invitations, firm_workspace_join_requests, firm_activity_log activity_type CHECK, inbox view `v_firm_user_pending_actions`) — reviewed before apply (no Critical/High), live leak-matrix + 50-child/1000-action load test (3.8ms) passed, types regen'd, 31 static guards green. **Key as-built:** the inbox view is owner-privileged (NOT security_invoker) — see `docs/PHASE_10_BUILD_SPEC.md` CP1 as-built ([D1]–[D4]) — because `lease_approval_chain`/`lease_unlock_requests` SELECT policies gate on direct `workspace_members`, not the firm-aware `is_workspace_member`. **CP2 done:** pure firm helpers — `firmAccess.ts` `isFirmAdminOrOwner`/`isFirmMemberOrOwner` (+ Deno mirror) + frontend `firmContext.ts` (resolveActiveFirm/computeActionUrgency/computeFirmSidebarMode). **CP3 done + DEPLOYED + smoked:** 9 new firm edge fns (6 invitation send/accept/get-info/list/revoke/resend + 2 join-request act-on/cancel + set-firm-access [workspace-owner-only restrict_firm_access toggle]) + #102-safe + #103 server guards on create-checkout/customer-portal/manage-document-pack (reject firm-bound, reason firm_managed) + stripe-webhook firm branch deployed (inert until firm subs exist). Security-reviewed (no Critical/High). **Deferred:** create-workspace firm_id + billing_summary_mode invoice line-items → CP4b (coupled to FirmOnboarding's Stripe path); delete-firm → Phase 11 (KNOWN_ISSUES #104 — firm_activity_log ON DELETE RESTRICT blocks hard-delete). **Remaining: CP4a** (FirmContext provider + Dashboard + Inbox + Members) → **CP4b** (FirmWorkspaces/Billing/Settings/Onboarding + sidebar/switcher/onboarding refactor + create-workspace firm_id + #103 UI gates + billing line-items) → **CP5** (regression + docs + closeout). Plan: `docs/PHASE_10_KICKOFF_PLAN.md`; column-level source: `docs/PHASE_10_BUILD_SPEC.md`.
