...
4. **Inbox view: plain vs. materialized.** Spec says plain view for v1, materialize only if scale demands. **Rec: plain view v1**, but build the 50-child / 1000-action load test into Checkpoint 1 so the decision is evidence-based. — **RESOLVED 2026-06-15 (CP1): plain view.** Load test on staging measured **3.8ms** for 1000 pending actions across 50 child workspaces, fully index-driven (`idx_lease_approval_chain_workspace_pending` + the new `idx_workspace_roles_workspace_user_role`); no materialization needed. Also discovered a 6th decision in CP1: the view is **owner-privileged (not security_invoker)** because `lease_approval_chain`/`lease_unlock_requests` SELECT policies gate on direct `workspace_members` (an invoker view would empty the inbox for firm-derived members); scoping is enforced airtight in the WHERE and validated by a live leak matrix.
