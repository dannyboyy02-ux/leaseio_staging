...
### Notification rail — `recipient_ids` fanout (follow-on from #111 C6c review) — RESOLVED (in-app) 2026-06-18

> **Branch `claude/notification-rail-followup` (off `main`), pending merge.** Assign a #-number at merge (the #108–#122 range is used by the unfinished-audit branches; this item was surfaced later by the #111 C6c review, not the original audit).
...
**Follow-ons (filed, NOT bundled):**
- **Email delivery (Medium).** This is IN-APP only (`notifications` has no email path; `process-alerts` writes it in-app, `send-lease-notifications` emails only the lease-owner calendar rail). Approver/delegation notifications still don't email. Needs a dispatcher for the `notifications` table.
- **`send-counter-signature-reminder` sends NO email despite its name (Medium).** Confirmed during the rail mapping — it only writes a (now-fanned) dead comment row + bumps a counter; no Resend import. Counter-sig chasing is non-functional as an *email* reminder even after this fix.
