...
### Item #132: P3 audit items C2 + D6 — C2 RESOLVED, D6 still deferred
...
### Item #133: `LeaseReview` has a `useEffect` after early returns (rules-of-hooks) — pre-existing, from the #116 archive-deep-link change

> **Filed 2026-06-22** (branch `claude/affectionate-hamilton-bp58tu`). Surfaced by lease-code-auditor during the C2 review as a **HIGH**, but it is NOT C2 — it predates C2 and was introduced by the `#116` archive-deep-link change on this same branch. Filed separately per the "pre-existing issues are their own beat" rule rather than bundled into the C2 commit.

- **(HIGH — react-hooks/rules-of-hooks) `src/pages/app/LeaseReview.tsx` (the `?action=archive` deep-link `useEffect`, ~line 2040) sits AFTER the `if (loading) return` / `if (isProcessing) return` early returns.** Every other hook in this component is above the first early return; this one is conditionally called, so when the component transitions out of loading/processing the hook order changes and React can throw or misfire effects. ESLint reports `react-hooks/rules-of-hooks` on it. **Root cause:** the archive-deep-link effect was added low in the component body (next to the archive-dialog state it drives) instead of with the other effects up top. **Fix:** hoist the effect above the `if (loading) return` (place it with the other `useEffect`s), guarding the body with `if (!lease) return;` so it still no-ops until the lease loads. Verify with `npx eslint src/pages/app/LeaseReview.tsx` that the rules-of-hooks error clears.
