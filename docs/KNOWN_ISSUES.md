...
**Follow-on chunks (not in this branch):**
- **C4 ▶ — escalate-to-concept re-resolve.** PIVOT FINDING: the audit's "reuse the reroute path" is **invalidated** — `resolve-approval-chain` reroute mode is change-gated (returns `no_reroute_needed` when structured attributes are unchanged), so reusing it would make escalate a **no-op in exactly the C4 caveat case**. The real bug: `escalate-to-concept-approver` clones concept rows of **any status** (no filter) → resurrects **superseded prior-policy approvers**. Correct fix = add an unconditional **"force concept reactivation" mode** to `resolve-approval-chain` (reuses its inline `matchPolicy` + the new atomic reconcile); escalate invokes it. Sizable, integrity-critical → its own chunk + integrity review.
- **C6 ▶ — per-policy SLA.** Decided (Daniel 2026-06-18): add `approval_policies.sla_days` (default 7); surface SLA-aware aging to the BLOCKING APPROVER in `ApprovalQueue` (not just the admin ExceptionsDashboard) + a nudge/notify-admin action (reuse the #109 nudge infra). Multi-surface: migration + SLA helper (Node⇄Deno mirror) + `ChainStepBadges` + `ApprovalQueue` + policy-editor input + i18n.
