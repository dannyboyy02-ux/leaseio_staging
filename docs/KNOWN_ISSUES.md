...
- **(HIGH — reporting fidelity) `src/components/reports/RentRollExport.tsx:89,106` uses only `current_monthly_rent || 0`.** The rent-roll CSV — an artifact finance teams hand to auditors — ignores `executed_monthly_payment`, `monthly_payment`, and `rent_schedules`. For an executed lease whose rent lives in `executed_monthly_payment` with `current_monthly_rent` null (the executed-document path), the export writes **0** for Monthly/Annual Rent and understates the portfolio-summary footer totals (lines ~129-130). Pre-existing; out of B3's frontend scope. **Root cause:** single-field read instead of the canonical chain. **Fix:** route through `getMonthlyRent(lease)` (the query is `.select('*')`, so a `rent_schedules` relation can be added if schedule-awareness is wanted). Best folded into the **B4** RentRoll pass (P3), which already touches this file's status filter. Surfaced to owner 2026-06-21.
