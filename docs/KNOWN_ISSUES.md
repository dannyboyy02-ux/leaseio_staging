...
> **RESOLVED in code 2026-06-18 (branch `claude/approval-jargon-fix`) — pending merge.** R1: extracted a shared, tested `src/lib/csv.ts` `escapeCsvCell()` that neutralizes formula-injection prefixes (`=`/`+`/`-`/`@`/tab/CR → leading `'`) on top of RFC-4180 quoting, and routed RentRollExport through it (use it for any future CSV export). R2: the query now filters `lifecycle_status` (committed states: approved/in_negotiation/final_review/executed/fully_executed/pending_counter_signature/active) + `archived=false` instead of the legacy `status` column. R4: "Total Annual Obligation" → "Total Annual Rent (run-rate)". Frontend-only (no deploy gate); 6 csv tests + typecheck green. **NOTE — the lifecycle set is a product choice** (whether a rent roll should include pre-execution pipeline like approved/in_negotiation, or executed+active only); trivially tunable in one place. Delete on merge.
