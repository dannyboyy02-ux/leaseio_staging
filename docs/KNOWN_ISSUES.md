...
### Item #132: P3 audit items C2 + D6 — deferred to a focused session

> **Filed 2026-06-22** (branch `claude/affectionate-hamilton-bp58tu`). Deferred from the P3 remediation pass: C2 because it's a high-risk refactor of the core approval-submission path (CLAUDE.md flags approval tickets for extra care + 3+ review rounds), D6 because it's a zero-visible-impact DRY refactor whose churn isn't justified at the tail of a long session. The other P3 items (B4/B5/C4/C5/D4/D5) shipped.

- **(MED — dead-end) C2: a request whose approval routing failed sits in `draft` with no retry.** `LeaseRequestForm.tsx` inserts the lease as `lifecycle_status='draft'` + `intake_source='request_workflow'`, then calls `resolve-approval-chain`; if `decideSubmissionOutcome` returns `leave_draft` (ambiguous match / no_match_no_fallback / separation_violation / network error), it toasts and returns, leaving the lease stuck in draft. On LeaseReview there's no "Retry routing" CTA, so a transient failure looks permanent and users may re-upload (duplicates). **The mechanism already exists** — `resolve-approval-chain` is idempotent on `initialResolution=true` (see the as-built comment at `LeaseRequestForm.tsx:360`). **Precise plan:** (1) detection condition is `intake_source === 'request_workflow' && lifecycle_status === 'draft'` (direct uploads use `manual_upload` and never reach `draft`; `submitted_for_approval_at` is null for a stuck draft); (2) the retry must re-run resolve→`decideSubmissionOutcome`→apply-flip→notify→log, which LeaseRequestForm does INLINE (lines 348–424) — so the clean fix is to **extract that finalize block into a shared helper** (e.g. `finalizeRequestRouting(supabase, lease, workspace, user)` recomputing `approvalRequirements` via `getApprovalRequirements` from the lease record), then call it from BOTH LeaseRequestForm and a new LeaseReview "Retry routing" banner; (3) heavy review (integrity + the approval subsystem). Do NOT duplicate the finalize logic — extract it. Read `docs/APPROVAL_ROUTING_ARCHITECTURE.md` first.
- **(LOW — consistency) D6: the six `/app/firm/*` pages hand-roll their page header.** Each renders `<div className="flex items-center gap-2"><{Icon} className="h-5 w-5 text-primary" /><h1 className="text-2xl font-semibold">{title}</h1></div>` (+ optional subtitle / role badge / right-side actions). They're already **visually consistent** — this is code duplication, not visual drift (hence Low). **Plan:** extract `FirmPageHeader({ icon, title, subtitle?, badge?, actions? })` (wrap in `flex items-start justify-between gap-4`; FirmDashboard is the superset — title + role badge + right-side Inbox action) and apply to FirmDashboard/Billing/Inbox/Members/Workspaces/Settings. Pure presentation; confirm no layout drift with a polish pass.
