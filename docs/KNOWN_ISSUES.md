...
- **`multiple_permissive_policies` (~90, WARN).** Multiple PERMISSIVE policies for the same role+action on a table each get evaluated per query. Heaviest on `firm_invitations` (24) + `firm_members` (24), plus alert_rules/approval_chain_steps/approval_policies/invite_tokens/lease_approval_chain/profiles/workspace_approvers (6 each). **This is a structural consolidation (merging policies / converting to a single policy with OR'd predicates), with real semantic risk** — must be done per-table with security review BEFORE apply, NOT a mechanical sweep. Each merge must preserve the exact union of what the separate policies admitted.
