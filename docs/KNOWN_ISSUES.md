...
- **#173 (MEDIUM, polish) raw Postgres/RLS errors piped into user toasts — RESOLVED 2026-07-18** (wave-1 commit `2c640d1`): shared `src/lib/userFacingError.ts` `mapSupabaseError()` (lock-trigger → `errors.lease_locked`, 42501/RLS → `errors.no_permission`, network → `errors.network`, else caller fallback; raw always console.error'd) routed through all 13 sites in VendorCard / LockedLeaseDetail / DisclosureReportLibrary / LeaseReportDetail; `font-mono` dropped from the now-localized inline errors (`userFacingError.test.ts`). The DB-stored `report.error_message` render is deliberately out of scope.
