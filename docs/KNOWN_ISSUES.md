...
## Fresh-eyes walkthrough fixes 2026-07-18 (branch `fresh-eyes-fixes`) — ~30 fixes shipped; these Tier-4 items deferred
...
- **#172 (MEDIUM, i18n) date/currency leaks on core surfaces** — `ApprovalQueue`/`SignatorReview` render dates via raw `date-fns format` (English months regardless of locale); `LeaseReportDetail` shows raw status/method tokens + browser-locale dates; `Portfolio` mixes hardcoded `$`/`M`/`K` with the localized formatter; the Leases CSV export is all-English. Fix: route through `formatLocalizedDate`/`formatLocalizedCurrency` + add keys. Deferred because it means threading `language` through nested queue sub-components (ChainStepCard/LeaseQueueCard) — cohesive follow-up, higher risk than its polish value right before a merge.
- **#173 (MEDIUM, polish) raw Postgres/RLS errors piped into user toasts** — `VendorCard`, `LockedLeaseDetail`, `DisclosureReportLibrary`, `LeaseReportDetail` surface `error.message`/`String(err)` (e.g. "new row violates row-level security policy…"). `EscalationReviewPanel` was fixed this pass (mapped). Fix: one shared error-mapper → localized fallback, keep raw in console only.
- **#174 (LOW, architectural) SPA-nav dirty guards** — LeaseReview, Asc842InputsTab, ApprovalPolicyEditPage guard only `beforeunload`; in-app navigation silently drops unsaved edits. Needs the `createBrowserRouter` data-router migration for a shared `useBlocker` guard (same lift as #169). Batch #169 + #174 together.
- **#175 (LOW, polish) divergent password policy** — self-signup + reset require length≥8 only; invited users (AcceptInvite) require upper+lower+number with a live checklist. The main path is the weaker one. Fix: one shared `passwordPolicy` helper.
- **#176 (LOW, marketing) Landing hero focal visual is an empty "demo coming" placeholder** (`HeroSection.tsx:61`, Sparkles + caption). Reads unfinished. Fix: drop in a real screenshot of the review workbench.
- **#177 (LOW) misc single-surface polish** — confidence-badge tiers (90/70) disagree with the 80 border + 80 flag cutoff (`LeaseReviewSections.tsx`); Portfolio "Cost per sqft by Location" is actually per-lease (two leases at one address = two bars); forecast card jargon ("re-leasing cliff", "uncontracted"); FirmBilling "detailed" invoice mode promises itemization deferred to #105; firm "Children" count excludes billed `restrict_firm_access` children; policy-list "Archive" == the row Switch (both `is_active=false`) + a native `window.confirm`; policy editor "Try sample" tests saved not draft; reset-password has no expired-link state; RentScheduleTable "Next increase" labels a flat/decreasing step as an increase; expiring-tile "mark seen" is a no-undo per-device localStorage dismiss; "Approve" is invisible until each flagged field is individually clicked (jump should scroll+focus).
- **#178 (LOW, integrity) escalation review of a finalized/locked lease** routes nowhere useful — the dashboard EscalationReviewPanel correctly rejects a `model_locked` lease with a mapped "request an unlock" message, but there's no in-panel affordance to start that governance flow. Correct-by-design (locked financial data changes through unlock→change-set), filed for a future "resolve via governance" link.
