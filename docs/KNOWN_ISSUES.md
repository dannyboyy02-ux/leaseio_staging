...
- **#A2 — PARTLY RESOLVED 2026-06-23.** The governance-bypass buttons "Move to Under Review/Approved" + "Mark Executed" were REMOVED (`LeaseReview.tsx`) — they skipped the approval queue/chain + the executed-doc upload and were trigger-rejected anyway. **Still OPEN:** "Cancel Request" and "Edit & Resubmit" do blocked browser writes (cancel needs a server action; resubmit resets trigger-guarded approval columns + re-routes) → to be fixed server-side together with #A3.
