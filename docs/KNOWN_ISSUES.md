...
- **Mixed-locale on partially-i18n'd surfaces (pre-existing, made slightly more visible).** Localizing dates exposed that some surfaces are otherwise hardcoded English: `src/pages/LeaseAudit.tsx` — the public lead-magnet card now shows a Spanish-formatted date next to an English `fmt()` currency under English labels ("Tenant"/"Landlord"/"Monthly Rent"); `src/components/dashboard/RecentActivity.tsx` — older feed rows show a localized date while "Today"/"Yesterday"/"N days ago" stay English. The date change is correct in isolation; these surfaces just need a full i18n pass (labels + currency + relative-time strings), not a date-only fix. **Fix:** fold each surface into i18n as its own pass; do not half-translate.
