-- Extend the lease_activity_log activity_type allowlist with
-- 'escalation_review_resolved', written by the dashboard EscalationReviewPanel
-- when a human confirms/clears a lease's escalation terms (attributable audit
-- of a financial-data edit). This is the latest definition of the CHECK — it
-- reproduces the full ARRAY from 20260625130000_lease_retention_lifecycle.sql
-- verbatim and appends only the new value.
...
ALTER TABLE public.lease_activity_log
  ...
  CHECK (activity_type = ANY (ARRAY[
    ...
    -- Dashboard escalation-review resolution (2026-07-17)
