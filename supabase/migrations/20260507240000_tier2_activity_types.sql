-- Tier 2 classification activity types — Phase 1 of the Tier 2 build.
--
-- Adds four new activity_type values to lease_activity_log:
--   - tier2_classification_passed       (Phase 1: Haiku gated, document is a lease)
--   - tier2_classification_rejected     (Phase 1: Haiku gated, hard rejection)
--   - tier2_classification_overridden   (Phase 5: user overrode a rejection)
--   - tier2_correction_recorded         (Phase 4: correction stored for ML learning)
--
-- The migration drops + recreates the activity_type CHECK constraint to
-- include the four new values alongside the existing 79. Idempotent:
-- DROP CONSTRAINT IF EXISTS guards re-apply.
--
-- Rationale for shipping all four enum values in Phase 1:
-- The CHECK constraint is the gate; defining a value before its writer
-- ships is harmless. Avoids 4 separate migrations across phases 1, 4, 5.
...
ALTER TABLE public.lease_activity_log
  ...
  CHECK (activity_type IN (
    ...
    -- Tier 2 classification (Phase 1+4+5 of the Tier 2 build)
