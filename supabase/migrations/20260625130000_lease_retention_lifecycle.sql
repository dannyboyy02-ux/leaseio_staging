...
-- ── 5. Audit activity types for the delete / restore / purge lifecycle ────────
-- The soft-delete, restore, and purge actions each write a lease_activity_log
-- row (server-side, service_role) so the deletion is attributable at the moment
-- it happens — separate from 'lease_archived'/'lease_restored' (which mean the
-- restorable-forever ARCHIVE, a different concept). A CHECK constraint can't be
-- appended to, so this re-DROPs and re-ADDs the full allowlist + the 3 new
-- values (same pattern as 20260623000000). The array below is the verbatim
-- current set plus 'lease_soft_deleted'/'lease_restored_from_deletion'/
-- 'lease_purged'. Keep this the latest definition if another migration extends it.
...
ALTER TABLE public.lease_activity_log
  ...
  CHECK (activity_type = ANY (ARRAY[
    ...
    -- Phase 3 lease soft-delete / 14-day retention (2026-06-25)
    'lease_soft_deleted','lease_restored_from_deletion','lease_purged'
  ]));
