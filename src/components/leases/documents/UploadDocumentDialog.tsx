  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (uploadError) {
        ...
        toast.error(t('documents.upload.upload_failed_with', { message: uploadError.message }));
        return;
      }
      ...
      if (invokeError || !(data as any)?.ok) {
        const msg =
          (data as any)?.error || invokeError?.message || t('documents.upload.record_failed');
        toast.error(msg);
      ...
      toast.success(
        t('documents.upload.uploaded', {
          iteration: result.document.iteration_number,
      ...
      toast.error(t('documents.upload.upload_failed'));
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="file-input">{t('documents.upload.file_label')}</Label>
            <Input
              id="file-input"
              type="file"
            ...
            {fileTooLarge && (
              <p className="text-xs text-destructive">
                {t('documents.upload.too_large')}
              </p>
            )}
          </div>

          <div className="space-y-2">
            <Label htmlFor="document-type">{t('documents.upload.type_label')}</Label>
            <Select
              value={documentType}
            ...
            >
              <SelectTrigger id="document-type">
                <SelectValue placeholder={t('documents.upload.type_placeholder')} />
              </SelectTrigger>
              <SelectContent>
                {allowed.map((dt) => (
                  <SelectItem key={dt} value={dt}>
                    {t(`documents.type.${dt}`, { defaultValue: documentTypeLabel(dt) })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            {allowed.length === 0 && (
              <p className="text-xs text-muted-foreground">
                {t('documents.upload.no_types')}
              </p>
            )}
            <p className="text-xs text-muted-foreground">
              {t('documents.upload.types_filtered')}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="notes">{t('documents.upload.notes_label')}</Label>
            <Textarea
              id="notes"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder={t('documents.upload.notes_placeholder')}
              rows={3}
              disabled={busy}
              maxLength={500}
            />
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!canSubmit}>
            ...
            {busy ? t('documents.upload.uploading') : t('common.upload')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
